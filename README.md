...
<sup>2</sup> - Prints the sequence in PLA+ (single colour) on a Klipper printer (256×256 mm bed). `run_all.ps1` skips it: there is no host `klipper` command, and a toolpath cannot hash-match `FizzBuzz.txt`.
